...
async def cmd_set_caption(bot: Client, msg: Message):
    if len(msg.command) < 2:
        await msg.reply_text(
            "**Usage :** `/set_caption Ton texte ici`\n\n"
            "**Variables disponibles :**\n"
            "`{filename}` — nom du fichier\n"
            "`{filesize}` — taille du fichier\n"
            "`{duration}` — durée (vidéo/audio)"
        )
        return
    caption = msg.text.split(None, 1)[1]
    await set_caption(msg.from_user.id, caption)
    await msg.reply_text(f"✅ **Légende sauvegardée !**\n\n`{caption}`")
...
async def cmd_see_caption(bot: Client, msg: Message):
    ...
    if not caption:
        await msg.reply_text("❌ **Aucune légende définie.**")
        return
    await msg.reply_text(f"📝 **Ta légende actuelle :**\n\n`{caption}`")


@Client.on_message(filters.private & filters.command("del_caption"))
async def cmd_del_caption(bot: Client, msg: Message):
    await del_caption(msg.from_user.id)
    await msg.reply_text("🗑️ **Légende supprimée !**")
...
async def cmd_set_prefix(bot: Client, msg: Message):
    if len(msg.command) < 2:
        await msg.reply_text("**Usage :** `/set_prefix TonPréfixe`")
        return
    prefix = msg.text.split(None, 1)[1]
    await set_prefix(msg.from_user.id, prefix)
    await msg.reply_text(f"✅ **Préfixe sauvegardé :** `{prefix}`")
...
async def cmd_see_prefix(bot: Client, msg: Message):
    ...
    if not prefix:
        await msg.reply_text("❌ **Aucun préfixe défini.**")
        return
    await msg.reply_text(f"🔤 **Ton préfixe actuel :** `{prefix}`")


@Client.on_message(filters.private & filters.command("del_prefix"))
async def cmd_del_prefix(bot: Client, msg: Message):
    await del_prefix(msg.from_user.id)
    await msg.reply_text("🗑️ **Préfixe supprimé !**")
...
async def cmd_set_suffix(bot: Client, msg: Message):
    if len(msg.command) < 2:
        await msg.reply_text("**Usage :** `/set_suffix TonSuffixe`")
        return
    suffix = msg.text.split(None, 1)[1]
    await set_suffix(msg.from_user.id, suffix)
    await msg.reply_text(f"✅ **Suffixe sauvegardé :** `{suffix}`")
...
async def cmd_see_suffix(bot: Client, msg: Message):
    ...
    if not suffix:
        await msg.reply_text("❌ **Aucun suffixe défini.**")
        return
    await msg.reply_text(f"🔤 **Ton suffixe actuel :** `{suffix}`")


@Client.on_message(filters.private & filters.command("del_suffix"))
async def cmd_del_suffix(bot: Client, msg: Message):
    await del_suffix(msg.from_user.id)
    await msg.reply_text("🗑️ **Suffixe supprimé !**")
